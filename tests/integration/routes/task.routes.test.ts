...
describe("Task Routes - Integration Tests", () => {
  ...
  describe("PATCH /api/tasks/:taskId/toggle-archived", () => {
    it("should toggle task from not archived to archived", async () => {
      const user = await createTestUser();
      const token = generateAuthToken(user.id, user.email);
      const task = await createTestTask(user.id, { archived: false });

      const response = await request(app)
        .patch(`/api/tasks/${task.id}/toggle-archived`)
        .set("Authorization", `Bearer ${token}`)
        .expect(200);

      expect(response.body.success).toBe(true);
      expect(response.body.message).toBe(
        "Task archived status toggled successfully",
      );
      expect(response.body.data.archived).toBe(true);
    });

    it("should toggle task from archived to not archived", async () => {
      const user = await createTestUser();
      const token = generateAuthToken(user.id, user.email);
    ...
    it("should return 404 for non-existent task", async () => {
      ...
      const response = await request(app)
        .patch("/api/tasks/99999/toggle-archived")
        .set("Authorization", `Bearer ${token}`)
        .expect(404);
